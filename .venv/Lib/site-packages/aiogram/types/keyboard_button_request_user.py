...
class KeyboardButtonRequestUser(TelegramObject):
    """
    This object defines the criteria used to request a suitable user. The identifier of the selected user will be shared with the bot when the corresponding button is pressed. `More about requesting users Р В РІР‚в„ўР вЂ™Р’В» <https://core.telegram.org/bots/features#chat-and-user-selection>`_
