...
def truncate(v: Union[str], *, max_len: int = 80) -> str:
    ...
    if isinstance(v, str) and len(v) > (max_len - 2):
        # -3 so quote + string + Р В Р вЂ Р В РІР‚С™Р вЂ™Р’В¦ + quote has correct length
        return (v[: (max_len - 3)] + 'Р В Р вЂ Р В РІР‚С™Р вЂ™Р’В¦').__repr__()
    try:
        v = v.__repr__()
    except TypeError:
        v = v.__class__.__repr__(v)  # in case v is a type
    if len(v) > max_len:
        v = v[: max_len - 1] + 'Р В Р вЂ Р В РІР‚С™Р вЂ™Р’В¦'
    return v
