...
class TestBuildExt(TempdirManager):
    ...
    def test_unicode_module_names(self):
        modules = [
            ...
            Extension('fР В РІР‚СљР вЂ™Р’В¶Р В РІР‚СљР вЂ™Р’В¶', ['uuu'], optional=False),
        ]
        dist = Distribution({'name': 'xx', 'ext_modules': modules})
        cmd = self.build_ext(dist)
        cmd.ensure_finalized()
        assert re.search(r'foo(_d)?\..*', cmd.get_ext_filename(modules[0].name))
        assert re.search(r'fР В РІР‚СљР вЂ™Р’В¶Р В РІР‚СљР вЂ™Р’В¶(_d)?\..*', cmd.get_ext_filename(modules[1].name))
    ...
    def test_export_symbols__init__(self):
        ...
        modules = [
            ...
            Extension('fР В РІР‚СљР вЂ™Р’В¶Р В РІР‚СљР вЂ™Р’В¶.__init__', ['uuu']),
        ]
        dist = Distribution({'name': 'xx', 'ext_modules': modules})
        cmd = self.build_ext(dist)
        cmd.ensure_finalized()
