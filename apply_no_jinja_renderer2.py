# apply_no_jinja_renderer2.py — перезаписывает src/result_renderer.py (без Jinja2)
...
def render_html_report(code: str, vectors: dict, features: dict, neighbors):
    ...
    {css}</head><body><div class='wrap'>
      ...
      <div class='card'><h2>Близкие профили</h2><p>{_tags(neigh)}</p></div>
    </div></body></html>"""
    return html
'''
Path("src/result_renderer.py").write_text(code, encoding="utf-8")
print("OK: src/result_renderer.py перезаписан без Jinja2")
