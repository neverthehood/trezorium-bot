# hotfix_renderer_no_jinja.py — заменяет src/result_renderer.py на версию без Jinja2
import pathlib
p = pathlib.Path("src/result_renderer.py")
p.write_text("""from typing import Optional, List, Tuple
...
def _tags(items: List[str]) -> str:
    if not items: return ""
    return "".join(f"<span class=\\"tag\\">{_esc(str(i))}</span>" for i in items)
...
def render_html_report(code: str, vectors: dict, features: dict, neighbors):
    ...
    one_liner = item.get("one_liner", "Коротко о твоём ведущем способе действовать.")
    description = item.get("description", "Этот профиль описывает, как ты обычно организуешь задачи, идеи, людей и практику.")

    strengths = item.get("strengths", ["Стратегичность","Способность собирать картину"])
    pitfalls  = item.get("pitfalls",  ["Перебор контроля","Риск затянуть обсуждение"])
    grow      = item.get("grow",      ["Гибкость","Быстрые пробы маленькими шагами"])
    ...
    return html
""", encoding="utf-8")
print("OK: result_renderer.py заменён (вёрстка без Jinja2).")
